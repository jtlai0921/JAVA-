...
public class ch27_14 {	
	static Frame frm = new Frame("ch27_14");
	static Label xlab = new Label();					// 標籤紀錄x軸
	static Label ylab = new Label();					// 標籤紀錄y軸
	static Label lab = new Label();						// 紀錄事件
// 擔任事件傾聽者和擁有事件處理者
	static class myListener implements MouseMotionListener{	// 實作MouseMotionListener
		...
		public void mouseMoved(MouseEvent e) { 			// mouseMoved事件處理者
			xlab.setText("x = " + e.getX());			// 輸出x座標
			ylab.setText("y = " + e.getY());			// 輸出y座標
			lab.setText("Mouse Moved");					// 輸出滑鼠被移動
		}
	}
